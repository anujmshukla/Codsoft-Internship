...
# # Fetching images and encoding (converting images into numerical data) for all images we have  
# Steve_Jobs_Image = face_recognition.load_image_file("Faces_Data/Steve_Jobs.png")
# Steve_Jobs_Encoding = face_recognition.face_encodings(Steve_Jobs_Image)[0]

# Ratan_Tata_Image = face_recognition.load_image_file("Faces_Data/Ratan_Tata.jpeg")
# Ratan_Tata_Encoding = face_recognition.face_encodings(Ratan_Tata_Image)[0]

# Mukesh_Ambani_Image = face_recognition.load_image_file("Faces_Data/Mukesh_Ambani.jpg")
# Mukesh_Ambani_Encoding = face_recognition.face_encodings(Mukesh_Ambani_Image)[0]

# Elon_Musk_Image = face_recognition.load_image_file("Faces_Data/Elon_Musk.jpeg")
    ...
    if Condition:
        ...
        for faceEncoding in face_encodings:
            ...
            if name in known_face_name:
                #hear it will cheeck that name is present in the Known_Faces (which is copy of known_face_name) then perform further opration otherwise not   
                if name in Known_Faces:
                    # now once "name" is found then simplly remove it from the Known_Faces so that if that face appear again the don't mark it again 
                    Known_Faces.remove(name)
                    print(Known_Faces)
                    current_time=now.strftime("%H-%M-%S")
                    #wirte data in csv file 
